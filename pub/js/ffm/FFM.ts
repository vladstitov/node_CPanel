module cpanel {
    ...
    export class FFM {
        ...
        private onTIEnter(evt) {
            if (evt.charCode == 13) this.getCurrentFolderDara();
        }
        private getCurrentFolderDara(){
...
*/
        ...
        private onBackClick(evt: JQueryEventObject): void {
            var folder: string = this.header.val();
            var ind: number = folder.lastIndexOf('/');
            folder = folder.substr(0, ind);            
            this.header.val(folder);

        }
        private onFolderDoubleClick(evt): void {
        ...
        constructor(private viewPort) {
            ...
            var backBtn: JQuery = $('<div>').addClass('btnBack').text('<-').on(CLICK, null, (evt) => this.onBackClick(evt));
           ...
           this.folderView.dblclick('.folder', (evt) => this.onFolderDoubleClick(evt));
        }
